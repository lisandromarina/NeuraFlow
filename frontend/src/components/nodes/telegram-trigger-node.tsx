...
interface TelegramTriggerNodeProps {
  data: {
    content: string;
    [key: string]: any;
  };
...
const TelegramTriggerNode: React.FC<TelegramTriggerNodeProps> = memo(({ data }) => {
  return (
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <TelegramIcon className="w-5 h-5 text-blue-500 cursor-pointer" />
                </TooltipTrigger>
                <TooltipContent>
                  <p>{data.actualType}</p>
                </TooltipContent>
